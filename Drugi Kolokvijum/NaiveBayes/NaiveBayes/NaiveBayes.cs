    ...
    public class NaiveBayes
    {
        ...
        public void predict(string text)
        {
            ...
            foreach (KeyValuePair<string, int> item in counts)
            {
                ...
                double neg = (count_neg + 1) / (word_counts[0].Values.Sum() + wordCount);
                double pos = (count_pos + 1) / (word_counts[1].Values.Sum() + wordCount);
                log_prob_neg += Math.Log(neg);
                log_prob_pos += Math.Log(pos);
