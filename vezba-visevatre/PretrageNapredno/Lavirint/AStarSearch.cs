...
namespace Lavirint
{
    class AStarSearch
    {
        ...
        {
            double r = Math.Sqrt(Math.Pow(s.markI - Main.krajnjeStanje.markI, 2) + Math.Pow(s.markJ - Main.krajnjeStanje.markJ, 2))+s.cost;
            for(int i=0; i<Main.vatra.Count; i++)
            {
                r += 4 / Math.Sqrt(Math.Pow(s.markI - Main.vatra[i].markI, 2) + Math.Pow(s.markJ - Main.vatra[i].markJ, 2));
            }
            return r;
        }
