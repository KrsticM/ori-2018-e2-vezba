...
namespace Lavirint
{
    class AStarSearch
    {
        ...
        public double heuristicFunction(State s)
        {
            ...
            for (int i = 0; i < Main.vatrapolja.Count; i++)
            {
                double rastojanje = Math.Sqrt(Math.Pow(s.markI - Main.vatrapolja[i].X, 2) + Math.Pow(s.markJ - Main.vatrapolja[i].Y, 2));
                if (rastojanje == 0)
                {
                    temp += 10000;
                }
            ...
            return temp;
        }// ovde promenimo heuristiku da bezi od vatre
